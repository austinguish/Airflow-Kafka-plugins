...
class EventMessageCRUD:
    ...
    def update_msgs(self, msg_list):
        ...
        str_msg_list = list(map(get_string_if_json, msg_list))
        del_record_list = [r for r in exist_records if r.msg not in str_msg_list]
        del_ids = [r.id for r in del_record_list]
    ...
    def have_successed_msgs(self, received_msgs):
        ...
        '''
        str_received_msgs = list(map(get_string_if_json, received_msgs))
        return [json.loads(v.msg) for v in self.session.query(EventMessage).filter(
                and_(
                    EventMessage.name == self.sensor_name,
